$(document).ready(function(){
    ...
    function createPieChart(){	//top 5 filters
    	...
    	for (var i = 0; i < filtersData.length; i++){
    		...
    		tmpObj.key = filtersData[i].filter;
    		tmpObj.count = filtersData[i].count;

    		keys.push(tmpObj);
    	}

    	var done = keys.sort(function(obj1, obj2) {
    	...
    	for (var i = 0; i < 5; i++){
    		...
    		tmpObj.filter = keys[i].key;
    		tmpObj.count = keys[i].count;
...
var margin = {top: 30, right: 120, bottom: 0, left: 120},
    width = $("#mainContainer").width() - margin.left - margin.right,
...
barChartData = "{ 'name': '" + test + "', 'children': [";
        ...
        for (var i = 0; i < filtersData.length; i++){
          if (i == filtersData.length - 1){
            ...
            if (filtersData[i].hashTags.length > 0){
            	barChartData = barChartData + ", 'children': [ ";
            	for (var j = 0; j < filtersData[i].hashTags.length; j++){
            		if (j == filtersData[i].hashTags.length - 1){
            			barChartData = barChartData + "{'name': '" + filtersData[i].hashTags[j].tag + "', 'size': " + filtersData[i].count + "}]}";
            		} else {
            			barChartData = barChartData + "{'name': '" + filtersData[i].hashTags[j].tag + "', 'size': " + filtersData[i].count + "}, ";
            		}
            	}
            } else {
            	barChartData = barChartData + ", 'size': " + filtersData[i].count + "}]}";
            }
